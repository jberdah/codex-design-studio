...
export function SlidePreview({ slide, tokens, brandName, index, active, onClick }: { slide: SlideSpec; tokens: DesignTokens; brandName: string; index: number; active: boolean; onClick: () => void }) {
  const style = { "--p": tokens.colors.primary, "--s": tokens.colors.secondary, "--a": tokens.colors.accent, "--bg": tokens.colors.background, "--text": tokens.colors.text, "--display": tokens.typography.display, "--body": tokens.typography.body } as React.CSSProperties;
